...
def AR_step(prev_state):
    ...
    ar_step = State.update(ar_step, "TP_x", 1 * State.TP_x(prev_state))
    return ar_step
...
def shock_step_random(prev_state):
    ...
    # update z 
    z_values = tf.cast(sample_index, dtype=global_default_dtype)
    # shock values
    shock_vals = tf.reshape(tf.gather(shock_values,sample_index),[prev_state.shape[0],shock_values.shape[1]])

    # kappa values (set shock to 0 if absorbing state reached)
    kappa_values = shock_vals[:,0]
    # kappa shock should be 0 if kappa is below epsilon
    kappa_values = tf.where(State.kappa_x(prev_state) < 1e-4, 0.0, kappa_values)
 
    # TP shock should be 0 if TP is reached
...
def shock_step_spec_shock(prev_state, shock_index):
    ...
    # kappa values (set shock to 0 if kappa_old is below 0.001)
...
def policy_step(prev_state, policy_state):
    ...
    policy_step = State.update(policy_step, "tcomp_x", Definitions.tcomp2tcompplus(prev_state, policy_state)) # update time
