...
def tau_x(state, policy_state):
    _tau = State.tau_const_x(state) + State.tau_S_x(state) * State.S_x(state)/0.851 + State.tau_kappa_x(state) * State.kappa_x(state)/0.35032 + State.tau_TP_x(state) * (1.0-(tf.maximum(State.TP_x(state) - State.Temp_x(state),0.0)/(3.5-1.4467)))
...
for i in range(1,Parameters.N + 1):
    setattr(sys.modules[__name__], "transfer" + str(i), (lambda ind: lambda s, ps: getattr(State, "tshare" + str(ind) + "_x")(s) * tax_income(s,ps))(i))  
...
# "damage" function damage would be 1 - omega
