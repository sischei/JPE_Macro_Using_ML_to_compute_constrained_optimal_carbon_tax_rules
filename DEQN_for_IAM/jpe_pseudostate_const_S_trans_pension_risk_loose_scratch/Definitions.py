...
def mu_y(state, policy_state=None):
    _mu = ((tau_x(state, policy_state) * State.kappa_x(state)) / (State.Omega_x(state) * theta1(state, policy_state) * theta2) ) ** (1/(theta2-1)) 
    return _mu
...
def tau_x(state, policy_state):
    ...
    _tau = tf.clip_by_value(_tau, 1e-7, 2.2) # clip tax
    return  _tau
...
# transfer definitions (uniform for now)
for i in range(1,Parameters.N + 1):
    setattr(sys.modules[__name__], "transfer" + str(i), (lambda ind: lambda s, ps: getattr(Parameters, "t" + str(ind)) * tax_income(s,ps))(i)) # growth in transfers 
