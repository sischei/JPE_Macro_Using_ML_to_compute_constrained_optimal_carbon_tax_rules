...
constants = {'alpha': 0.3, 
            'beta': 0.9, # 0.8, discount factor approx 0.95 per year 
            'sigma': 3.0, # risk aversion
            'delta': 0.2, # approx. 0.1 per year # lowDelta = 0.2
            ...
            'kappaT_range': (0.05,), # Final emissions
            'tpT_range': (3.0,), # distance from tipping (in BAU we have approx 3 degrees of warming)
            'param_bounds': np.array([[-1.0, 0.5],[-0.5, 1.],[-0.6, 0.6],[-1., 1.]]), # np.array([[-2.0, 2.0]] * 4), # Parameter bounds (arbitrary)
...
tax_states.append({'name': 'tau_kappa_x'})
tax_states.append({'name': 'tau_TP_x'})
tax_states.append({'name': 'tau_x'})

transfer_states = []
# transfer shares. Note that the Nth is redundant
for i in range(1, constants['N']+1):
...
for i in range(1, constants['N']):
    asset_dict = {'name': 'anext{}_y'.format(i), 'bounds': {'lower': -0.05, 'upper': 0.2}, 'activation': 'implied'} # , 'bounds': {'lower': 1e-3}, 'penalty_lower': 10000.0
...
# my_activation = "lambda x: custom_activation(x)"
# VFs
VF_y = []
for i in range(1,constants['N']):
...
# global_policies.append({'name': 'KKTmu_y', 'activation': 'tf.nn.softplus'})

# global_policies.append({'name': 'mu_y', 'activation': 'tf.nn.sigmoid'})
# examples to alternate, as sigmoid has issues with diminishing gradients at times.
#global_policies.append({'name': 'mu_y', 'activation': 'tf.nn.softplus', 'bounds':{'upper':1., 'penalty_upper':1e6}}) 
#policies_xi.append({'name':'muy','activation': 'tf.keras.activations.sigmoid', 'bounds':{'lower':0.01, 'penalty_lower':1e6, 'upper':1., 'penalty_upper':1e-2}})
#policies_xi.append({'name':'muy','activation': 'tf.keras.activations.linear', 'bounds':{'lower':0.0, 'penalty_lower':1e1, 'upper':1., 'penalty_upper':1e1}})
...
for i in range(1, constants['N']+1):
    cons = {'name': 'c{}'.format(i), 'bounds': {'lower': 1e-3}} # , 'penalty_lower': 10000.0
