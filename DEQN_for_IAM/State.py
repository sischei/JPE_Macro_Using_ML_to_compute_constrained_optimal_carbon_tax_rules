...
def E_t_gen(state, policy_state, training=None):
    if expectation_type != 'pseudo_random':
        ...
        next_policies = [policy(next_state, training=training) for next_state in next_states]
...
def E_t_gen_double(state, policy_state, training=None):
    if expectation_type != 'pseudo_random':
        next_states = [Dynamics.total_step_spec_shock(state, policy_state, i) for i in range(len(Dynamics.shock_probs))]
        next_policies = [policy(next_state, training=training) for next_state in next_states]
