...
def mu_y(state, policy_state=None):
    # tf.print("numerator",tau_x(state, policy_state) * State.kappa_x(state))
    # tf.print("denominator",State.Omega_x(state) * theta1(state, policy_state) * theta2)
...
# Damages as a fraction of total GDP (as total output also includes nondepreciated capital)
