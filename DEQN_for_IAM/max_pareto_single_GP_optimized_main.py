...
torch.set_num_threads(1)  # Also set in MAIN process, not just workers
...
KERNEL_CHOICE = 'MA52' # Can be changed, but make sure pretrained model exists for the chosen kernel
...
if Parameters.MODEL_NAME in ['jpe_pseudostate_S_transfers_risk_test','jpe_pseudostate_S_transfers_risk_test_kappa', 'jpe_pseudostate_S_transfers_risk_test_4theta']:
...
# transfers should sum to 1 constraint
def sum_transfers(x):
...
# transfer bounds taken from dirichlet distribution
...
if FINAL_OPTIMIZATION:
    ...
    # param_names = [f"param_{i}" for i in range(len(candidates[0]))]
